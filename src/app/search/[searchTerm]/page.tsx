...
export default async function SearchPage(searchParams: PageParams) {
  const { params: { searchTerm }} = searchParams;
  ...
  try {
    ...
    return <main className="max-w-6xl m-auto flex justify-between px-3 py-6 items-center">
      {
        results.length > 0 ? <Cards cards={results} /> : <h1>No movies found</h1> }
    </main>
  } catch (error) {
